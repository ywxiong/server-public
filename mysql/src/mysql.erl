        ...
        {ok, ConnPid} ->
            ...
            case gen_server:call(
                ...
                ok ->
                    {ok, ConnPid};
                Res ->
                        catch erlang:exit(ConnPid, kill),
                    error_logger:error_msg("mysql add_conn: ~p", [Res]),
                    Res
            end;
        Err ->
            error_logger:error_msg("mysql start_conn:~p", [Err]),
            Err
    end.
...
fetch(PoolId, Query, Timeout) ->
    case get(?STATE_VAR) of
        undefined ->
            case ?SQL of
                1 ->
                    io:format("fetch ~p ~p ~n ", [PoolId, Query]);
                _ ->
                    skip
            end,
...
handle_info({'DOWN', _MonitorRef, process, Pid, Info}, State) ->
    error_logger:warning_msg("mysql DOWN: ~p", [Pid]),
    LogFun = State#state.log_fun,
...
add_conn(Conn, State) ->
    ...
    error_logger:warning_msg("mysql add_conn ~p", [Conn#conn.pid]),
