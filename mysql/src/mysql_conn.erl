...
post_start(Pid, LogFun) ->
    ...
    after 30000 ->
        ...
        error_logger:error_msg("mysql post_start timed out:~p", [Pid]),
        {error, "timed out"}
    end.
...
do_recv(LogFun, RecvPid, SeqNum) when is_function(LogFun);
    LogFun == undefined,
    SeqNum == undefined ->
    %%io:format("mysql_recv1 closed reason=~p ~n LogFun~p",[SeqNum,LogFun]),
    receive
        {mysql_recv, RecvPid, data, Packet, Num} ->
            {ok, Packet, Num};
        {mysql_recv, RecvPid, closed, E} ->
            error_logger:warning_msg("mysql_recv closed reason=~p", [{RecvPid,E,self()}]),
            error_logger:warning_msg("mysql_dispatcher =~p", [sys:get_status(whereis(mysql_dispatcher))]),
            exit(self(),kill),
            {error, "mysql_recv: socket was closed"}
    %%{error,closed,RecvPid,E}
    end;
do_recv(LogFun, RecvPid, SeqNum) when is_function(LogFun);
    LogFun == undefined,
    is_integer(SeqNum) ->
    io:format("mysql_recv2 closed reason=~p", [SeqNum]),
...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
    ...
    case do_send(Sock, Packet, 0, LogFun) of
        ...
        {error, Reason} ->
            error_logger:warning_msg("mysql Failed sending data on socket : ~p",[{RecvPid,self(),Reason,Query}]),
            error_logger:warning_msg("mysql_dispatcher =~p", [sys:get_status(whereis(mysql_dispatcher))]),
            exit(self(), kill),
